...
The DC power modules (25 kW bricks) communicate with the CM5 main controller over a dedicated CAN bus (CAN #1). This interface handles voltage/current setpoint control, module enable/disable, status telemetry, and fault reporting. Each module contains its own DSP/MCU that runs closed-loop power conversion; the CM5 acts as the bus master, issuing high-level setpoints and monitoring aggregate output.
...
| **Isolation** | 3 kV galvanic isolation (isolated CAN transceiver on CM5 side) |
| **Termination** | 120 Ω at each end (CM5 controller + last module on bus) |
| **Cable** | Shielded twisted pair, daisy-chain topology |
| **Max Bus Length** | 20 m (at 500 kbps, all modules within single cabinet) |
| **Max Nodes** | 12 modules + 1 master (CM5) |
...
```
CM5 (Master)           Module 1           Module 2           Module N
  ┌───┐                 ┌───┐              ┌───┐              ┌───┐
  │120Ω│                │   │              │   │              │120Ω│
  └─┬─┘                 │   │              │   │              └─┬─┘
────┴───────────────────┴───┴──────────────┴───┴──────────────┴──── CAN_H
...
Daisy-chain wiring: CM5 → Module 1 → Module 2 → ... → Module N
Termination only at CM5 and last module on the chain.
```
...
|---------|------------|-------|
| `0x00` | CM5 Main Controller (bus master) | Source address for all commands |
...
|----------|-----------|-------|-------------|
| `0x010` – `0x01F` | CM5 → Module | Command | Setpoints, enable/disable |
| `0x0F0` – `0x0FF` | CM5 → All | Command (broadcast) | Global commands |
| `0x110` – `0x1CF` | Module → CM5 | Status | Per-module state and measurements |
| `0x210` – `0x2CF` | Module → CM5 | Telemetry | Temperatures, efficiency, runtime |
| `0x310` – `0x3CF` | Module → CM5 | Fault | Per-module fault codes and details |

### 4.2 Command Messages (CM5 → Module)
...
This message is sent by the CM5 when EvseManager calls `powersupply_DC.setMode(Off)` or upon receiving a fault from the safety supervisor. All modules must disable output within 10 ms of receiving this frame.

### 4.3 Status Messages (Module → CM5)
...
**Cycle time:** 100 ms per module. With 6 modules, the CM5 receives ~60 status messages/second on CAN #1.
...
### 4.4 Telemetry Messages (Module → CM5)
...
Each module sends a heartbeat frame at 1000 ms intervals. The CM5 monitors all modules and flags any module that misses 3 consecutive heartbeats as offline.
...
1. PREPARE       CM5 sends: Mode = Standby, Enable = 1 (per module)
                 Modules: DC link charges, output remains off
                 Status: each module reports state = 0x02 (Standby)

2. PRECHARGE     CM5 sends: Mode = Precharge, V = target, I = 2A
                 (broadcast 0x0F0)
                 Modules: one designated module ramps voltage to target
                 CM5 monitors: V_out approaches V_target via status 0x110

3. FULL CHARGE   CM5 sends: Mode = CV, V = target, I = per-module limit
                 (broadcast 0x0F0)
...
All modules connected to the shared DC output bus operate in droop-mode current sharing, coordinated by the CM5:
...
If Module 3 is derated to 70% (thermal):
  ...
  CM5 recalculates and re-sends individual setpoints
```

The CM5 recalculates current distribution whenever:
...
2. CM5 receives fault, removes Module 4 from active pool
3. CM5 recalculates: 5 remaining modules carry the load
4. If total demand > 5-module capacity:
   CM5 enables standby module (Module 7, Priority → 0, Enable → 1)
5. CM5 sends updated current limits to all active modules
...
|---------|-----------|-------|--------|------------|
| Setpoint (broadcast) | CM5 → All | 1 | 100 ms | 10 |
| Module Control | CM5 → Individual | ~2 | On-change | ~2 |
| Module Status | Module → CM5 | 6 | 100 ms | 60 |
| Power Measurement | Module → CM5 | 6 | 1000 ms | 6 |
| Thermal Telemetry | Module → CM5 | 6 | 2000 ms | 3 |
| Electrical Telemetry | Module → CM5 | 6 | 2000 ms | 3 |
| Heartbeat | Both | 7 | 1000 ms | 7 |
| Fault (worst case) | Module → CM5 | 6 | 1000 ms | 6 |
| **Total** | | | | **~97** |
...
| Module fault → CM5 notification | 100 ms | Next status cycle |
| Module fault → current redistribution | 200 ms | CM5 recalculate + broadcast new setpoints |
...
```
1. CM5 powers on, initializes CAN #1 interface
2. CM5 sends master heartbeat (0x700) at 1000 ms intervals
...
4. CM5 detects modules via heartbeat, builds active module list
5. CM5 sends Module Control with Enable = 0, Priority per config
6. Modules transition to NMT state = 0x05 (operational), report Ready (0x01)
7. CM5 reports capabilities to EvseManager via power_supply_DC interface
```
...
```
1. Module heartbeat lost → CM5 marks module as offline
2. Current redistributed across remaining modules
3. Module heartbeat returns (NMT = 0x00, then 0x7F)
4. CM5 sends Enable = 0, queries module health via status
...
The power module CAN bus (CAN #1) is managed by the CM5, but the safety supervisor on CAN #2 has indirect authority:
...
│                                                              │
│  2. CM5 watchdog: if CM5 dies, safety supervisor opens       │
